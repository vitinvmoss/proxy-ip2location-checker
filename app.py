from flask import Flask, render_template, request, jsonify, redirect, url_for, make_response
from werkzeug.middleware.proxy_fix import ProxyFix
import hmac
import hashlib
import time
import secrets
import requests
import os
import re
from urllib.parse import quote
from concurrent.futures import ThreadPoolExecutor, as_completed

app = Flask(__name__)
app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_proto=1, x_host=1)

SECRET = os.environ.get("FLASK_SECRET_KEY", "").strip() or secrets.token_urlsafe(48)
APP_PASSWORD = os.environ.get("APP_PASSWORD", "").strip()
AUTH_COOKIE = "proxy_checker_auth"
AUTH_MAX_AGE = 60 * 60 * 24 * 30

# Tuned for the user's normal ~20-proxy runs.
MAX_PROXIES = 50
MAX_WORKERS = 20
MAX_LOOKUP_WORKERS = 20

IP_ECHO_URL = "https://api.ipify.org?format=json"
IP2LOCATION_URL = "https://api.ip2location.io/"
IP2LOCATION_BULK_URL = "https://bulk.ip2location.io/"


def parse_proxy(line: str):
    """Parse host:port:user:pass, user:pass@host:port, or scheme://user:pass@host:port."""
    line = line.strip()
    if not line:
        return None

    raw = line
    m = re.match(r"^(?:(https?|socks5h?|socks4)://)?(.+)$", line, re.I)
    if not m:
        return None

    scheme = (m.group(1) or "").lower()
    body = m.group(2)

    if "@" in body:
        creds, hostport = body.rsplit("@", 1)
        if ":" in creds:
            username, password = creds.split(":", 1)
        else:
            username, password = creds, ""
    else:
        parts = body.split(":")
        if len(parts) < 4:
            return None
        if len(parts) == 4:
            host, port, username, password = parts
        else:
            host = ":".join(parts[:-3])
            port, username, password = parts[-3:]
        hostport = f"{host}:{port}"

    if hostport.startswith("["):
        end = hostport.find("]")
        if end == -1 or end + 1 >= len(hostport) or hostport[end + 1] != ":":
            return None
        host = hostport[1:end]
        port_s = hostport[end + 2:]
    else:
        if ":" not in hostport:
            return None
        host, port_s = hostport.rsplit(":", 1)

    try:
        port = int(port_s)
        if not 1 <= port <= 65535:
            return None
    except ValueError:
        return None

    return {
        "raw": raw,
        "scheme": scheme,
        "host": host,
        "port": port,
        "username": username,
        "password": password,
    }


def proxy_url(p, scheme):
    user = quote(p["username"], safe="")
    password = quote(p["password"], safe="")
    host = p["host"]
    if ":" in host and not host.startswith("["):
        host = f"[{host}]"
    return f"{scheme}://{user}:{password}@{host}:{p['port']}"


def detect_exit_ip(p):
    """Connect through the supplied proxy and ask ipify which IP it sees."""
    declared = p["scheme"]
    if declared:
        if declared in ("socks5", "socks5h"):
            schemes = ["socks5h"]
        elif declared == "socks4":
            schemes = ["socks4"]
        else:
            schemes = ["http"]
    else:
        schemes = ["http", "socks5h"]

    last_error = None
    for sch in schemes:
        try:
            url = proxy_url(p, sch)
            proxies = {"http": url, "https": url}
            r = requests.get(IP_ECHO_URL, proxies=proxies, timeout=(6, 12))
            r.raise_for_status()
            data = r.json()
            ip = data.get("ip")
            if not ip:
                raise RuntimeError("IP echo service returned no IP")
            return {
                "exit_ip": ip,
                "proxy_type": sch,
                "response_ms": r.elapsed.total_seconds() * 1000,
            }
        except Exception as exc:
            last_error = str(exc)

    raise RuntimeError(last_error or "Proxy connection failed")


def ip2location(ip):
    """Single-IP IP2Location.io lookup. Uses the user's free API key when configured."""
    params = {"ip": ip, "format": "json"}
    key = os.environ.get("IP2LOCATION_API_KEY", "").strip()
    if key:
        params["key"] = key

    r = requests.get(IP2LOCATION_URL, params=params, timeout=(5, 10))
    r.raise_for_status()
    data = r.json()

    if isinstance(data, dict) and data.get("error"):
        err = data["error"]
        if isinstance(err, dict):
            raise RuntimeError(str(err.get("error_message") or err))
        raise RuntimeError(str(err))

    return data


def ip2location_bulk(ips):
    """Optional paid bulk endpoint. Not used unless explicitly enabled."""
    key = os.environ.get("IP2LOCATION_API_KEY", "").strip()
    if not key or os.environ.get("IP2LOCATION_BULK_ENABLED", "").lower() not in ("1", "true", "yes"):
        raise RuntimeError("Bulk endpoint disabled")

    params = {"key": key, "format": "json"}
    r = requests.post(
        IP2LOCATION_BULK_URL,
        params=params,
        json=ips,
        timeout=(5, 20),
    )
    r.raise_for_status()
    data = r.json()
    if isinstance(data, dict) and data.get("error"):
        err = data["error"]
        raise RuntimeError(str(err.get("error_message") if isinstance(err, dict) else err))
    return data


def check_proxy(index, line):
    p = parse_proxy(line)
    base = {
        "index": index,
        "proxy": line,
        "status": "failed",
        "source_proxy_check": "ipify.org via supplied proxy",
    }
    if not p:
        base["error"] = "Unsupported proxy format. Use host:port:user:pass."
        return base

    try:
        detected = detect_exit_ip(p)
        base.update({
            "status": "online",
            "exit_ip": detected["exit_ip"],
            "proxy_type": detected["proxy_type"],
            "response_ms": round(detected["response_ms"]),
        })
        return base
    except Exception as exc:
        base["error"] = str(exc)
        return base


def apply_location(result, data):
    proxy = data.get("proxy") if isinstance(data.get("proxy"), dict) else {}
    as_info = data.get("as_info") if isinstance(data.get("as_info"), dict) else {}

    result.update({
        "country": data.get("country_name", ""),
        "country_code": data.get("country_code", ""),
        "region": data.get("region_name", ""),
        "city": data.get("city_name", ""),
        "district": data.get("district", ""),
        "latitude": data.get("latitude"),
        "longitude": data.get("longitude"),
        "zip_code": data.get("zip_code", ""),
        "time_zone": data.get("time_zone", ""),
        "isp": data.get("isp", ""),
        "asn": data.get("asn", "") or as_info.get("as_number", ""),
        "as_name": data.get("as", "") or as_info.get("as_name", ""),
        "as_domain": as_info.get("as_domain", ""),
        "usage_type": data.get("usage_type", "") or as_info.get("as_usage_type", ""),
        "is_proxy": data.get("is_proxy"),
        "fraud_score": data.get("fraud_score"),
        "proxy_last_seen": proxy.get("last_seen"),
        "proxy_detection_type": proxy.get("proxy_type", ""),
        "proxy_threat": proxy.get("threat", ""),
        "proxy_provider": proxy.get("provider", ""),
        "is_vpn": proxy.get("is_vpn"),
        "is_tor": proxy.get("is_tor"),
        "is_data_center": proxy.get("is_data_center"),
        "is_public_proxy": proxy.get("is_public_proxy"),
        "is_web_proxy": proxy.get("is_web_proxy"),
        "is_web_crawler": proxy.get("is_web_crawler"),
        "is_ai_crawler": proxy.get("is_ai_crawler"),
        "is_residential_proxy": proxy.get("is_residential_proxy"),
        "is_spammer": proxy.get("is_spammer"),
        "is_scanner": proxy.get("is_scanner"),
        "is_botnet": proxy.get("is_botnet"),
        "is_bogon": proxy.get("is_bogon"),
        "source_location": "IP2Location.io",
    })


def make_auth_token():
    ts = str(int(time.time()))
    sig = hmac.new(SECRET.encode(), ts.encode(), hashlib.sha256).hexdigest()
    return f"{ts}.{sig}"


def valid_auth_token(token):
    try:
        ts_s, sig = token.split(".", 1)
        ts = int(ts_s)
        now = time.time()
        if now - ts > AUTH_MAX_AGE or ts > now + 60:
            return False
        expected = hmac.new(SECRET.encode(), ts_s.encode(), hashlib.sha256).hexdigest()
        return hmac.compare_digest(sig, expected)
    except (ValueError, TypeError):
        return False


def is_authenticated():
    return valid_auth_token(request.cookies.get(AUTH_COOKIE, ""))


@app.before_request
def require_password():
    if not APP_PASSWORD:
        return None
    if request.endpoint in ("login", "login_post", "health", "static"):
        return None
    if is_authenticated():
        return None
    if request.path.startswith("/api/"):
        return jsonify({"error": "Authentication required."}), 401
    return redirect(url_for("login"))


@app.get("/health")
def health():
    # Intentionally public so GitHub Actions/Uptime monitors can keep a free
    # Render instance warm without needing the private app password.
    return jsonify({"status": "ok"})


@app.get("/login")
def login():
    return render_template("login.html")


@app.post("/login")
def login_post():
    password = request.form.get("password", "")
    if password and hmac.compare_digest(password, APP_PASSWORD):
        response = make_response(redirect(url_for("index")))
        response.set_cookie(
            AUTH_COOKIE,
            make_auth_token(),
            max_age=AUTH_MAX_AGE,
            secure=True,
            httponly=True,
            samesite="Lax",
            path="/",
        )
        return response
    return render_template("login.html", error="Incorrect password."), 401


@app.get("/logout")
def logout():
    response = make_response(redirect(url_for("login")))
    response.delete_cookie(AUTH_COOKIE, path="/")
    return response


@app.get("/")
def index():
    return render_template("index.html")


@app.post("/api/check")
def api_check():
    payload = request.get_json(silent=True) or {}
    text = payload.get("text", "")
    if not isinstance(text, str):
        return jsonify({"error": "Invalid proxy list."}), 400

    lines = list(dict.fromkeys(x.strip() for x in text.splitlines() if x.strip()))
    if len(lines) > MAX_PROXIES:
        return jsonify({"error": f"Maximum {MAX_PROXIES} proxies per run."}), 400
    if not lines:
        return jsonify({"error": "No proxies supplied."}), 400

    # Stage 1: all proxy checks run concurrently.
    results = [None] * len(lines)
    with ThreadPoolExecutor(max_workers=min(MAX_WORKERS, len(lines))) as executor:
        futures = {executor.submit(check_proxy, i, line): i for i, line in enumerate(lines)}
        for future in as_completed(futures):
            i = futures[future]
            try:
                results[i] = future.result()
            except Exception as exc:
                results[i] = {
                    "index": i,
                    "proxy": lines[i],
                    "status": "failed",
                    "error": str(exc),
                }

    # Stage 2: only unique detected IPs are looked up, concurrently.
    unique_ips = list(dict.fromkeys(
        r.get("exit_ip")
        for r in results
        if r and r.get("status") == "online" and r.get("exit_ip")
    ))

    location_cache = {}

    # The documented IP2Location.io bulk endpoint requires a paid plan.
    # For the free plan, use parallel single-IP requests instead.
    use_bulk = (
        bool(os.environ.get("IP2LOCATION_API_KEY", "").strip())
        and os.environ.get("IP2LOCATION_BULK_ENABLED", "").lower() in ("1", "true", "yes")
        and len(unique_ips) > 1
    )

    if use_bulk:
        try:
            bulk_data = ip2location_bulk(unique_ips)
            if isinstance(bulk_data, dict):
                for ip in unique_ips:
                    if ip in bulk_data:
                        location_cache[ip] = {"ok": True, "data": bulk_data[ip]}
                    else:
                        location_cache[ip] = {"ok": False, "error": "IP missing from bulk response"}
        except Exception as exc:
            # Fall back to normal free/core lookups if bulk fails.
            for ip in unique_ips:
                location_cache[ip] = {"ok": False, "error": str(exc)}

    pending = [ip for ip in unique_ips if ip not in location_cache]
    if pending:
        with ThreadPoolExecutor(max_workers=min(MAX_LOOKUP_WORKERS, len(pending))) as executor:
            futures = {executor.submit(ip2location, ip): ip for ip in pending}
            for future in as_completed(futures):
                ip = futures[future]
                try:
                    location_cache[ip] = {"ok": True, "data": future.result()}
                except Exception as exc:
                    location_cache[ip] = {"ok": False, "error": str(exc)}

    for result in results:
        if not result or result.get("status") != "online":
            continue
        item = location_cache.get(result.get("exit_ip"))
        if item and item.get("ok"):
            apply_location(result, item["data"])
        elif item:
            result["location_error"] = item.get("error", "IP2Location lookup failed")

    return jsonify({
        "results": results,
        "unique_ips_looked_up": len(unique_ips),
        "lookup_mode": "bulk" if use_bulk else "parallel-single",
    })


if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000, threaded=True)
